...
NOTE: Before version 3.0.0 trailing slashes were ignored. `router.get('/person', ...)` would match both `/person` and `/person/`.
If you need old behavior add optional trailing slash in pattern: `router.get('/person/?', ...)`
